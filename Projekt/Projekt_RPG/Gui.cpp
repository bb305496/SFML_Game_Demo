	...
	short unsigned id)
{
	...
	//Putting text in the center of button
...
void gui::Button::update(const sf::Vector2i& mousePositionWindow)
{
	//Update the boolenas for hover and pressed
...
{
	return this->activeElemnet->getId();
}
...
void gui::DropDownList::updateKeytime(const float& dtime)
{
	if (this->keytime < this->keytimeMax)
		this->keytime += 10.f * dtime;
}

void gui::DropDownList::update(const sf::Vector2i& mousePositionWindow, const float& dtime)
{
	this->updateKeytime(dtime);

	this->activeElemnet->update(mousePositionWindow);

	if (this->activeElemnet->isPressed() && this->getKeytime())
	{
		if (this->showList)
			this->showList = false;
		else
	...
	if (this->showList)
	{
		for (auto& i : this->list)
		{
			...
			{
				this->showList = false;
				this->activeElemnet->setText(i->getText());
				this->activeElemnet->setId(i->getId());
			}
		}
	}
...
{
	this->activeElemnet->render(target);
...
void gui::TextureSelector::updateKeytime(const float& dtime)
{
    if (this->keytime < this->keytimeMax)
        this->keytime += 10.f * dtime;
}

void gui::TextureSelector::update(const sf::Vector2i& mousePositionWindow, const float& dtime)
{
    this->updateKeytime(dtime);
    this->hide_btn->update(mousePositionWindow);
    ...
    if (!this->hidden)
    {
        ...
        if (this->bounds.getGlobalBounds().contains(static_cast<sf::Vector2f>(mousePositionWindow)))
        {
            ...
            this->mousePositionGrid.x = (mousePositionWindow.x - static_cast<int>(this->bounds.getPosition().x)) / static_cast<unsigned>(this->gridSize);
            this->mousePositionGrid.y = (mousePositionWindow.y - static_cast<int>(this->bounds.getPosition().y)) / static_cast<unsigned>(this->gridSize);
...
)
{
        ...
        if (font)
        {
            ...
            this->text.setPosition(
                this->inner.getPosition().x + gui::p2pX(0.2604166f, vm), 
                this->inner.getPosition().y - gui::p2pY(0.5037f, vm));
        }
}
...
void gui::ProgressBar::update(const int current_value)
{
        ...
        this->barString = std::to_string(current_value) + "/" +
