...
TileMap::TileMap(float gridSize, int width, int height, std::string texture_file)
{
    ...
    {
        std::cout << "ERROR::TILEMAP::FAILED TO LOAD TEXTURE" << "\n";
    }
...
void TileMap::saveToFile(const std::string file_name)
{
    ...
    */
    ...
    if (of.is_open())
    {
        of << this->maxSizeWorldGrid.x << " " << this->maxSizeWorldGrid.y << "\n"
            << this->gridSizeI << "\n"
            << this->layers << "\n"
            << this->textureFile << "\n";
    ...
    {
        std::cout << "ERROR::TILEMAP:: Could not save tilemap to file";
    }
...
void TileMap::loadFromFile(const std::string file_name)
{
    ...
    if (i_f.is_open())
    {
        ...
        //Basic
        i_f >> size.x >> size.y >> gridSize >> layers >> texture_file;
        ...
        while (i_f >> x >> y >> z >> type)
        {
            if (type == TileTypes::ENEMYSPAWNER)
            {
                ...
                i_f >> trX >> trY >> enemy_type >> enemy_ammont >> enemy_tts >> enemy_md;
            ...
            else if (type == TileTypes::DAMAGING)
            {
                int damage = 2;
                i_f >> trX >> trY >> damage;

                this->map[x][y][z].push_back(new DamagingTile(
                    x, y,
                    this->gridSizeF,
                    this->tileSheet,
                    sf::IntRect(trX, trY, this->gridSizeI, this->gridSizeI),
    ...
    {
        std::cout << "ERROR::TILEMAP:: Could not load tilemap from file";
    }
...
void TileMap::updateWorldBoundsCollision(Entity* entity, const float& dtime)
{
...
void TileMap::updateTileColision(Entity* entity, const float& dtime)
{
    this->layer = 0;
    ...
    for (int x = this->fromX; x < this->toX; x++)
    {
        for (int y = this->fromY; y < this->toY; y++)
        {
            for (int k = 0; k < this->map[x][y][this->layer].size(); k++)
            {
                ...
                sf::FloatRect nextPositionBounds = entity->getNextPositionBounds(dtime);
                ...
                {
                    DamagingTile* damagingTile = dynamic_cast<DamagingTile*>(this->map[x][y][this->layer][k]);
                    if (damagingTile && damagingTile->intersects(playerBounds) && player->canTakeDamage())
                    {
                        player->loseHP(damagingTile->getDamage());
                    }
                }
            }
        }
...
void TileMap::updateTilesAsync(Entity* entity, const float& dtime)
{
    std::thread updateThread(&TileMap::updateTileColision, this, entity, dtime);
    updateThread.detach();
}

void TileMap::updateTiles(Entity* entity, const float& dtime, EnemySystem& enemySystem)
{
    this->layer = 0;
            ...
            for (int k = 0; k < this->map[x][y][this->layer].size(); k++)
            {
                ...
                if (this->map[x][y][this->layer][k]->getType() == TileTypes::ENEMYSPAWNER)
                {
                    ...
                    if (es)
                    {
                        ...
                        {
                            enemySystem.createEnemy(es->getType(), x * this->gridSizeF, y * this->gridSizeF, *es);
                        }
                    }
                }
            }
...
void TileMap::update(Entity* entity, const float& dtime)
{
    std::lock_guard<std::mutex> lock(mapMutex);

    this->updateTileColision(entity, dtime);
}
...
void TileMap::addDamagingTile(const int x, const int y, const int z, const sf::IntRect& texture_rect, int damage)
{
    if (x < this->maxSizeWorldGrid.x && x >= 0 && y < this->maxSizeWorldGrid.y && y >= 0 && z < this->layers && z >= 0)
    {
        this->map[x][y][z].push_back(new DamagingTile(x, y, this->gridSizeF, this->tileSheet, texture_rect, damage));
    }
}
