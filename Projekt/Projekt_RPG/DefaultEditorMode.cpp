...
void DefaultEditorMode::updateKeybinds(const float& dtime)
{
    //Add tile
    if (sf::Mouse::isButtonPressed(::sf::Mouse::Left) && this->getKeytime())
    {
        if (!this->sidebar.getGlobalBounds().contains(sf::Vector2f(*this->editorStateData->mousePositionWindow)))
        {
            if (!this->textureSelector->getActive())
            {
                if (this->tileAddLock)
                {
                    if (this->tileMap->tileEmpty(this->editorStateData->mousePositionGrid->x, this->editorStateData->mousePositionGrid->y, 0))
                    {
                        this->tileMap->addTile(this->editorStateData->mousePositionGrid->x, this->editorStateData->mousePositionGrid->y, 0, this->textureRect, this->collision, this->type);
                    }
                }
                else
                {
    ...
    else if (sf::Mouse::isButtonPressed(::sf::Mouse::Right) && this->getKeytime())
    {
        ...
        {
            if (!this->textureSelector->getActive())
                this->tileMap->removeTile(this->editorStateData->mousePositionGrid->x, this->editorStateData->mousePositionGrid->y, 0);
        }
    }
...
void DefaultEditorMode::updateGui(const float& dtime)
{
    this->textureSelector->update(*this->editorStateData->mousePositionWindow, dtime);
    if (!this->textureSelector->getActive())
    {
        this->selectorRect.setTextureRect(this->textureRect);
        //this->selectorRect.setTexture(this->tileMap->getTileSheet());
    ...
    ss << this->editorStateData->mousePositionView->x << " " << this->editorStateData->mousePositionView->y <<
        "\n" << this->editorStateData->mousePositionGrid->x << " " << this->editorStateData->mousePositionGrid->y <<
        "\n" << this->textureRect.left << this->textureRect.top <<
        "\n" << "Collision: " << this->collision <<
        "\n" << "Type: " << this->type <<
        "\n" << "Tiles: " << this->tileMap->getLayerSize(this->editorStateData->mousePositionGrid->x, this->editorStateData->mousePositionGrid->y, this->layer) <<
...
void DefaultEditorMode::update(const float& dtime)
{
