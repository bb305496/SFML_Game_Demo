...
void TextTagSystem::initFonts(std::string font_file)
{
    if (!this->font.loadFromFile(font_file))
        std::cout << "ERROR::TEXTAGSYSTEM:: faild to load font " << "\n";
}

void TextTagSystem::initTagTemplates()
{
    this->tagTemplates[TagTypes::DEFAULT_TAG] = new TextTag(this->font, "DEF_TAG", 100.f, 100.f, 0.f, -1.f, sf::Color::White, 20, 100.f, 200.f, 500.f, 2);
    this->tagTemplates[TagTypes::NEGATIVE_TAG] = new TextTag(this->font, "NEG_TAG", 100.f, 100.f, 0.f, 1.f, sf::Color::Red, 30, 50.f, 200.f, 500.f, 2);
    this->tagTemplates[TagTypes::EXPERIENCE_TAG] = new TextTag(this->font, "EXP_TAG", 0.f, 0.f, 0.f, -1.f, sf::Color::Cyan, 30, 70.f, 250.f, 500.f, 2);
}
...
void TextTagSystem::update(const float& dtime)
{
    for (size_t i = 0; i < this->tags.size(); ++i)
    {
        this->tags[i]->update(dtime);
