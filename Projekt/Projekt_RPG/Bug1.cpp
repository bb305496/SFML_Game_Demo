...
void Bug1::initVariables()
{

}

void Bug1::initAnimations()
{

    this->animationComponent->addAnimation("IDLE", 25.f, 0, 0, 3, 0, 75, 75);
    this->animationComponent->addAnimation("WALK_LEFT", 18.f, 0, 1, 3, 1, 75, 75);
    this->animationComponent->addAnimation("WALK_RIGHT", 18.f, 0, 2, 3, 2, 75, 75);
...
Bug1::Bug1(float x, float y, sf::Texture& texture_sheet, EnemySpawnerTile& enemy_spawner_tile, Entity& player)
    : Enemy(enemy_spawner_tile)
{
    this->initVariables();
    this->initGUI();

    this->createHitboxComponent(this->sprite, 10.f, 5.f, 60.f, 60.f);
    ...
    //this->attributeComponent->strength = 10;
...
void Bug1::updateAnimation(const float& dtime)
{
...
void Bug1::update(const float& dtime, sf::Vector2f& mouse_pos_view, const sf::View& view)
{
...
void Bug1::render(sf::RenderTarget& target, sf::Shader* shader, const sf::Vector2f light_position, const bool show_hitbox)
{
    if (shader)
    {
