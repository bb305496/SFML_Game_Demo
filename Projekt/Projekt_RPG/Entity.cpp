...
const sf::Vector2f Entity::getCenter() const
{
    if (this->hitboxComponent)
        return this->hitboxComponent->getPosition() + 
        sf::Vector2f(
            this->hitboxComponent->getGlobalBunds().width / 2.f,
            this->hitboxComponent->getGlobalBunds().height / 2.f
            );
...
{
    if (this->hitboxComponent)
        return this->hitboxComponent->getGlobalBunds();

    return this->sprite.getGlobalBounds();
}

const sf::FloatRect Entity::getNextPositionBounds(const float& dtime) const
{
    if (this->hitboxComponent && this->movementComponent)
        return this->hitboxComponent->getNextPosition(this->movementComponent->getVelocity() * dtime);
...
void Entity::move(const float dir_x, const float dir_y, const float& dtime)
{
...
//void Entity::update(const float& dtime, sf::Vector2f& mouse_pos_view)
//{
//	
//}
//
